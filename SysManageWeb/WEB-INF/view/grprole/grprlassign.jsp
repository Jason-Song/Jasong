		...
		<div class="main-content">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">用户的角色组(<span id="sfId"></span>)</h3>
	        ...
	        "drawCallback": function (settings) {
	        	...
	        	$('.cbr-replaced').bind('click',function(e){
	        		...
	        		$('.cbr-replaced',xtr[3]).each(function(i,dem) {
	        			...
	        			var vflag = rowdata.assFlag?rowdata.assFlag:false;
	        			if (vflag != nval) {
	        				ischange = true;
	        			}
	...
	$('#bupdate').click( function () {
		...
		if (rowsObj[0]) {
			...
			for (var i = 0 ; i < rows.length; i++) {
				...
				var thisData = row.data();
				thisData.assFlag = (thisData.assFlag?thisData.assFlag:false);
				var ndata = WebUtils.deepCopy(thisData);
				var boxes = $('.cbr-replaced',row.node());
				ndata.assFlag = $(boxes[0]).hasClass('cbr-checked');
				changed.push( ndata );
			}
			WebUtils.ajaxSubmitJson('grprlassign/save/${staffId}',JSON.stringify(changed),function(result) {
				alert (result.msg);
				table.draw();
			});
		}
