...
public class CometUtil extends ConnectListener implements ServletContextListener {
    ...
    public boolean handleEvent(ConnectEvent connEvent){
        ...
        CacheManagerUtil.putContent(userId.toString(), connEvent);
        return true;
    }
    private void doCache(final CometConnection conn,String userId) {  
        if (userId != null) {  
            CacheManagerUtil.putContent(conn.getId(), String.valueOf(userId), Constant.EXPIRE_AFTER_ONE_HOUR);  
        }  
    }
    /**
     * 推送给所有的客户端
     * @param comet
     */
    public void pushToAll(CometPo comet){
        try {
            CometEngine engine = CometContext.getInstance().getEngine();
               //推送到所有客户端  
     ...
     * @param comet
     */
    public void pushTo(CometPo comet){
        try {
            ConnectEvent connEvent = (ConnectEvent) CacheManagerUtil.getContent(comet.getUserId()).getValue();
            final CometConnection conn = connEvent.getConn();
               //建立连接和用户的关系  
               doCache(conn,comet.getUserId());
               final String connId = conn.getId();
               CometEngine engine = CometContext.getInstance().getEngine();
               if (CacheManagerUtil.getContent(connId).isExpired()) {  
                   doCache(conn,comet.getUserId());  
               }
               //推送到指定的客户端  
