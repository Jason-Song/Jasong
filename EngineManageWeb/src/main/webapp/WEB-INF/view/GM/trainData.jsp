						...
						<div class="row" >
							<div class="col-md-12">
								<div class="form-group">
									...
									<select class="form-control" id="unit" name="unit" data-validate="required" data-message-required="此项必须选择."></select>
								</div>
							</div>
						</div>
						<div class="row">
   ...
   <div class="modal-dialog" >
				...
				<div id="parent">
					<div class="modal-body">
						<div class="row">
							...
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">测试数据占比</label>
				                    <input name="testRate" id="testRate" style='width:100%' data-validate="required"></input>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">分类数</label>
				                    <input name="classNum" id="classNum" style='width:100%' data-validate="required"></input>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">生成树个数</label>
				                    <input name="treeNum" id="treeNum" style='width:100%' data-validate="required"></input>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">特征子集选取策略</label>
				                    <input name="featureSubsetStrategy" id="featureSubsetStrategy" style='width:100%' data-validate="required"></input>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">纯度计算</label>
				                    <select name="impurity" id="impurity" style='width:100%' data-validate="required">
				                    	<option value="gini">Gini不纯度</option>
							...
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">树的最大深度</label>
				                    <input name="maxDepth" id="maxDepth" style='width:100%' data-validate="required"></input>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">特征最大装箱数</label>
				                    <input name="maxBins" id="maxBins" style='width:100%' data-validate="required"></input>
								</div>
							</div>
							<div class="col-md-6">
							  	<div class="form-group">
	...
	$(function(){
		...
		$.ajax({    
		    ...
		    "success" : function(data) { 
			    ...
			    $("#s_unit").append("<option value=''></option>"+opts);
			    $("#unit").append("<option value=''></option>"+opts);
		     }    
		});
	    $('#unit').select2({  
            placeholder: "选择部门",  
		...
		$('#trainbutton').click( function () {
			...
			if(data){
				$("#traindetail").on("shown.bs.modal",function(){
					...
					$("input[name='testRate']").val("0.3");
					$("input[name='classNum']").val("2");
					$("input[name='treeNum']").val("3");
					$("input[name='featureSubsetStrategy']").val("auto");
					$("input[name='impurity']").val("gini");
					$("input[name='maxDepth']").val("4");
					$("input[name='maxBins']").val("32");
				});
	...
	function submitDetail() {
		...
		if (form.valid()) {
			...
			$("#trainInfo").html("<div style='text-align:center;'><img src='../../images/loading.gif' /></div>");
			// $('smsg').modal('show');
			//$('#trainDesc').val("<img src='../../images/loading.gif' />");
			$.ajax({
	        		type:"post",
	        		data:data,
	        		url:"train",
	        		...
	        		success:function(dt){
	    			    //alert(opts);
	    				$("#trainInfo").html("<label class='control-label'>训练信息</label><div>"+dt.msg+"</div>");
	        		},
	        		error:function(e){
	        			var bb = e.data;  
	    			    var os = "";
	    				...
	    				$("#trainInfo").html("<label class='control-label'>训练信息</label><div>"+e.msg+os+"</div>");
	                }
	        	});
		}
		return false;	
