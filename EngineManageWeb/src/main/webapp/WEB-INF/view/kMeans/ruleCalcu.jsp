	...
	<div class="page-container">
		<div class="main-content">
			<div class="page-title">
				<div class="title-env">
					<h1 class="title">慧脑智库</h1>
					<p class="description">应用策略配置及已保存的模型，对业务数据进行策略选择</p>
				</div>
			</div>
				...
				<div class="panel-body" id="inputName">
					...
					<div class="row">
						...
						<div class="col-md-3">
							...
							<div class="form-group">
								<input class="form-control" id="s_filePos" placeholder="文件存储位置"></input>
							</div>
						</div>
						<div class="col-md-3">
...
<div class="modal fade" id="detail">
   <div class="modal-dialog" >
				...
				<div id="parent">
					<div class="modal-body">
						<div class="row" >
							<div class="col-md-12">
								<div class="form-group">
									<label for="unit" class="control-label">部门</label>
						...
						<div class="row">		
							<div class="col-md-12">
								<div class="form-group">
									...
									<textarea id="c_fileDesc" class="col-md-12" rows="4"></textarea>
								</div>
							</div>
						</div>
						<div class="row">		
	...
	$(function(){
	    ...
	    var initparams = {
	        ...
	        "columnDefs" : [{
                "render" : function (data, type, row) {
                	if(data=="0")return "0";
                	else return "<a href='recView?batchFileId="+row.batchFileId+"'>"+data+"</a>";
                },
                "targets" : [7]
	         },{
                "render" : function (data, type, row) {
                	if(data=="0")return "0";
                	else return "<a href='relView?batchFileId="+row.batchFileId+"'>"+data+"</a>";
                },
                "targets" : [8]
	         },{
	         	"render" : function (data, type, row) {
	         		...
	         		else if(data=="2")return "处理成功";
	         		else if(data=="3")return "处理失败";
                },
                "targets" : [9]
	         }]
	    };
		...
		$('#addbutton').click( function () {
			$('#detail').modal('show', {backdrop: 'static',action: "productDataAdd",title:"新增业务数据"});
		});
    ...
    $('#expbutton').click(function () {
    	...
    	if(data.length){
	    	jQuery('#disable_box').modal('show',{text:"请确认是否导出？"});
	       	$("#disable_ok").on("click",function(){
	    		...
	    		window.location.href='batchScreenExport?str=' + str;
	       	});
   		}else parent.WebUtils.alert("请选择要导出的记录！");
    });
	
	$('#insert_button').click(function() {
		var unit = $("#unit").val();
		console.log(unit+"xadsdfaf");
        ...
        var text = $("#file").val();
        if (text == "") {
        	parent.WebUtils.alert("请先上传xls或xlsx或txt文件...");
            return false;
        }
        if (!text.match(/.xls/i)&&!text.match(/.xlsx/i)&&!text.match(/.txt/i)) {
            return false;
        }
        uploadFile();
	});
    ...
    $("#file").change(
        function(ev) {
            ...
            if (!checkFileExt(fileExt)) {
            	parent.WebUtils.alert("上传的文件不是xls或xlsx或txt格式,请重新上传！");
                this.value = "";
                return false;
            }
    });
    ...
    $("#downbutton").click(function(){  
    	window.location.href='downTempFile';
   	}); 
    function checkFileExt(ext) {
        if (!ext.match(/.xls/i)&&!ext.match(/.xlsx/i)&&!ext.match(/.txt/i)) {
            return false;
        }
        return true;
    }
   ...
   //附件上传
    function uploadFile() {
    	...
    	var unitObj = document.getElementById("unit"); // 获取文件对象
    	var fileType = document.getElementById("fileType"); // 获取文件对象
    	var fileDesc = document.getElementById("c_fileDesc"); // 获取文件对象
    	oData.append("file", fileObj);  
    	oData.append("unit", unitObj.value);  
    	oData.append("fileType", fileType.value);  
    	oData.append("fileDesc", fileDesc.value);  
    	var oReq = new XMLHttpRequest();  
    	oReq.open( "POST", "batchScreenAdd" , true );  
    	oReq.onload = function(oEvent) {  
    	     if (oReq.status == 200) {  
		        $('#detail').modal('hide');
		    	parent.WebUtils.alert("上传格式文件成功！");
    	     } else {  
		        $('#detail').modal('hide');
	    		parent.WebUtils.alert("上传格式文件失败！");
	         }  
    	};  
    	oReq.send(oData);
    }
