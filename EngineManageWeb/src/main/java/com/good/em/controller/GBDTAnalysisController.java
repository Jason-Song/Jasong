...
public class GBDTAnalysisController {
    private static Logger logger = LoggerFactory.getLogger(GBDTAnalysisController.class);
	...
	public WebPageResult getSelectData(){
		...
		WebPageResult ret =null;
		try {
			dataList = modelAnalysisService.getSelectData("4");
			ret = new WebPageResult(dataList);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(MsgConstants.E0000, e);
